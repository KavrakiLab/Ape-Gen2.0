...
Template_alleles <- fread("Updated_template_information.csv")[, MHC] %>% unique
...
HELL <- function(c1, c2) {
  mean(sqrt(rowSums((sqrt(c1) - sqrt(c2))^2)) / sqrt(2))
}
...
extract_matrix <- function(freq_table, allele_list, Template_alleles, nmer) {
  ...
  similarity_matrix <- lapply(allele_list, function(x) {
    ...
    similarity <- c(similarity, rep(0, length(not_supported_alleles))) # Max possible euclidean distance 
    i <<- i + 1
  ...
  similarity_matrix[outer(rownames(similarity_matrix), colnames(similarity_matrix), "==")] <- 0
